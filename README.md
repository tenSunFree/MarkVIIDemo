# MarkVIIDemo

---

## Introduction  

Source of inspiration:  

Mark-VII  
https://github.com/daemon-001/Mark-VII

ChatGPT  
https://play.google.com/store/search?q=chatgpt&c=apps&hl=zh_TW  

This project is for learning and technical practice. 

---

## Preview  

<p align="left">
  <img src="https://i.postimg.cc/SNFr2cQt/Screenshot-20260127-172448.png" height="460"/>
</p> 

<p align="left">
  <img src="https://i.postimg.cc/VsM985BP/Screenshot-20260127-172515.png" height="460"/>
</p> 

<p align="left">
  <img src="https://i.postimg.cc/NM4s5xyN/367579.jpg" height="460"/>
</p> 

---

## Features   

---

## Tech Stack  

- Jetpack Compose + Kotlin  
  Android UI framework (Declarative UI, Material 3, state-driven rendering)    
- OpenRouter API  
  LLM routing layer (single endpoint access to multiple models, OpenAI-compatible Chat Completions)      

---

## Credits  
  
Mark-VII  
https://github.com/daemon-001/Mark-VII

ChatGPT  
https://play.google.com/store/search?q=chatgpt&c=apps&hl=zh_TW    

This project is created for independent learning and demonstration purposes.
Special thanks to the original author for their open-source contribution.

---

## Notes  

Image resources are for learning and purposes only. Please do not use them for commercial purposes. 

If there is any infringement, please contact me for removal. Thank you.  

---

## License  

This repository is intended for learning and demonstration.  

If you plan to open-source it, please choose a license and confirm third-party asset usage rights.  
